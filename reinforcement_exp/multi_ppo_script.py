...
from experiments import Multi_Action_AGENTS
import tqdm
...
def parse_args():
    ...
    parser.add_argument("--agent", type=str, default="ConvActorOnehot",
        help="Which agent")
...
if __name__ == "__main__":
    ...
    agent = Multi_Action_AGENTS[args.agent](args.mask_actions).to(device)
    optimizer = optim.Adam(agent.parameters(), lr=args.learning_rate, eps=1e-5)

    # ALGO Logic: Storage setup
    obs = torch.zeros((args.num_steps, args.num_envs) + envs.single_observation_space.shape).to(device).to(torch.int32)
    ...
    for update in tqdm.tqdm(range(1, num_updates + 1)):
        ...
        writer.add_scalar("charts/avg_episodic_length", np.mean(average_length), global_step)

...
def make_env(gym_id, seed, idx):
    def thunk():
        if gym_id == "Sudoku-nostop0":
            ...
            print(
                "For this Env args.cut_off_limit args.win_reward, args.use_random_starting_point, and args.fail_penalty will be ignored."
            )
...
def eval_greedy(agent):
    ...
    while not terminated:
        ...
        action = agent.get_greedy_action(obs).cpu().numpy()
...
if __name__ == "__main__":
    ...
    if not until_ever:
        ...
        process_bar = tqdm.tqdm(range(num_updates))
    ...
    try:
        ...
        while True:
            ...
            writer.add_scalar(
                "charts/SPS",
                int((global_step - old_global_step) / (time.time() - start_time)),
                global_step,
            )

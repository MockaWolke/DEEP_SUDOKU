...
args.data_set = "realistic_easy_data"

start_time = time.time()

RUNNAME = f"{args.model}_{start_time}"
...
val_df = get_data("val", "realistic_easy_data")
val_ds = get_tf_dataset(val_df)
val_ds = val_ds.batch(args.batchsize).prefetch(tf.data.AUTOTUNE)
...
hist = model.fit(train_ds, epochs = args.epochs, callbacks=callbacks, initial_epoch=0, validation_data= val_ds)
